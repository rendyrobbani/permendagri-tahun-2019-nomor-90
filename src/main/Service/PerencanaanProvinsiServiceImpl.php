...
use RendyRobbani\Keuangan\Repository\PerencanaanProvinsiLogRepository;
use RendyRobbani\Keuangan\Repository\PerencanaanProvinsiRepository;
...
final readonly class PerencanaanProvinsiServiceImpl implements PerencanaanProvinsiService
{
	/**
	 * @param Connection $connection
	 * @param PerencanaanProvinsiRepository $repository
	 * @param PerencanaanProvinsiLogRepository $logRepository
	 * @param string $referensi
	 * @param string $penetapan
	 */
	public function __construct(protected Connection                                     $connection,
	                            protected PerencanaanProvinsiRepository                  $repository,
	                            protected PerencanaanProvinsiLogRepository               $logRepository,
	...
	function fromXlsx(Worksheet $worksheet): void
	{
		try {
			...
			/** @var array<string, PerencanaanProvinsiEntity> $intoEntities */
			$intoEntities = [];

			/** @var PerencanaanProvinsiLogEntity[] $logEntities */
			$logEntities = [];
			...
			for ($rowNum = 4; $rowNum <= $worksheet->getHighestRow(); $rowNum++) {
				...
				$intoEntity = new PerencanaanProvinsiEntity();
				for ($i = 0; $i < 6; $i++) {
					$value = $rowValues[$i];
	...
	function intoXlsx(Worksheet $worksheet): void
	{
		...
		$worksheet->setTitle("B");

		$rowNum = 1;
		$worksheet->getRowDimension($rowNum)->setRowHeight(48);
		$worksheet->getCell([1, $rowNum])->setValueExplicit($worksheet->getTitle() . ".");
		$worksheet->getCell([2, $rowNum])->setValueExplicit("KLASIFIKASI, KODEFIKASI, DAN NOMENKLATUR PERENCANAAN PEMBANGUNAN DAN KEUANGAN DAERAH PROVINSI");
		...
		$worksheet->getCell([6, $rowNum])->setValueExplicit("NOMENKLATUR URUSAN PROVINSI");
		...
		foreach ($this->repository->findAll() as $entity) {
			if ($entity->kodeBidang == null) $rowNum++;
		...
		for ($j = 0; $j < 8; $j++) {
			$rowNum++;
			if ($j == 0) $worksheet->getCell([1, $rowNum])->setValueExplicit("Keterangan Kode Kekhususan:");
			else {
				$worksheet->mergeCells("A$rowNum:B$rowNum");
				$worksheet->getCell([1, $rowNum])->setValueExplicit(str_repeat("*", $j) . ($j > 1 ? ")" : ""));
				$worksheet->getStyle([1, $rowNum])->getNumberFormat()->setFormatCode("@ * \":\"");
				$worksheet->getCell([3, $rowNum])->setValueExplicit(match ($j) {
					1 => "Provinsi Papua & Papua Barat",
					2 => "Provinsi Papua",

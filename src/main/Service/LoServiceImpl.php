	...
	function fromXlsx(Worksheet $worksheet): void
	{
		try {
			...
			for ($rowNum = 1; $rowNum <= $worksheet->getHighestRow(); $rowNum++) {
				...
				if ($row0[0] == null || !preg_match("/^[7-8]+$/", $row0[0])) continue;
				...
				if (in_array($reading_row, [1083, 1085, 1088])) {
					$entity->kodeRekening4 = "02";
				}

				if (in_array($reading_row, [3926])) {
					$entity->kodeRekening6 = "06";
				}

				if (in_array($reading_row, [7162, 7418])) {
					$entity->kodeRekening6 = "002";
				}

				if (in_array($reading_row, [7430, 7433, 7436])) {
					$entity->kodeRekening5 = "03";
				}

				if (in_array($reading_row, [7526, 7681, 7689, 9294])) {
					$entity->kodeRekening6 = "002";
				}

				if (in_array($reading_row, [10065])) {
				...
				for ($level = 1; $level <= sizeof($splitID); $level++) {
					...
					if ($entity->kode != $checkID && !in_array($checkID, $cacheID)) {
						switch ($level) {
							case 1:
								throw new RekeningNotFoundException(RekeningNotFoundException::LO, RekeningNotFoundException::LEVEL_1, $checkID);
							case 2:
								throw new RekeningNotFoundException(RekeningNotFoundException::LO, RekeningNotFoundException::LEVEL_2, $checkID);
							case 3:
								throw new RekeningNotFoundException(RekeningNotFoundException::LO, RekeningNotFoundException::LEVEL_3, $checkID);
							case 4:
								throw new RekeningNotFoundException(RekeningNotFoundException::LO, RekeningNotFoundException::LEVEL_4, $checkID);
							case 5:
								throw new RekeningNotFoundException(RekeningNotFoundException::LO, RekeningNotFoundException::LEVEL_5, $checkID);
							case 6:
								throw new RekeningNotFoundException(RekeningNotFoundException::LO, RekeningNotFoundException::LEVEL_6, $checkID);
						}
					}
				}
				...
				if (in_array($entity->kode, $cacheID)) {
					if (in_array($reading_row, [7572, 8259])) {
						...
						switch (sizeof(explode(".", $entity->kode))) {
							case 1:
								throw new RekeningExistsException(RekeningExistsException::LO, RekeningExistsException::LEVEL_1, $entity->kode);
							case 2:
								throw new RekeningExistsException(RekeningExistsException::LO, RekeningExistsException::LEVEL_2, $entity->kode);
							case 3:
								throw new RekeningExistsException(RekeningExistsException::LO, RekeningExistsException::LEVEL_3, $entity->kode);
							case 4:
								throw new RekeningExistsException(RekeningExistsException::LO, RekeningExistsException::LEVEL_4, $entity->kode);
							case 5:
								throw new RekeningExistsException(RekeningExistsException::LO, RekeningExistsException::LEVEL_5, $entity->kode);
							case 6:
								throw new RekeningExistsException(RekeningExistsException::LO, RekeningExistsException::LEVEL_6, $entity->kode);
						}
					}
				}
